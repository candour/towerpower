...
| REF-010 | 2025-06-20 | Refactored `StallUpgradeManager.kt` using a declarative `StatScaler` architecture. Replaced the iterative `fold` loop and large `when` block with delegated scalers, simplified stat normalization in `applyUpgrade`, and improved maintainability of upgrade logic. | Resolved |
