...
| REF-014 | 2026-07-18 | Refactored `GameBoard.kt` rendering logic to use a zero-allocation (pooled) layered architecture. Introduced explicit Background, Decals, World, and Foreground passes. Optimized World layer depth sorting by using a stable `sortWith(compareBy({ it.r }, { it.q }))` on pooled entities. | Resolved |
