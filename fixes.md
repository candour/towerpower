...
| REF-004 | 2025-05-27 | Refactored `GameBoard.kt` to use a layered rendering architecture (Background, Decal, World, Foreground). Optimized performance by introducing a `RenderingContext` to reuse drawing parameters and simplified the rendering loop to only sort the World layer by row coordinate. | Resolved |
