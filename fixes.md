...
| REF-014 | 2026-07-18 | Refactored `GameBoard.kt` rendering logic to use a zero-allocation (pooled) layered architecture. Introduced explicit Background, Decals, World, and Foreground passes. Optimized World layer depth sorting by using a stable `sortWith` with a singleton `WorldItemComparator` (sorting by r, zOrder, then q) on pooled entities. | Resolved |
| FIX-016 | 2026-07-20 | Fixed inconsistent Tiger Mom buff cleanup by centralizing logic into `cleanupEnemyBuffs` and ensuring it runs in both the enemy pipeline and projectile processing. | Resolved |
...
| REF-018 | 2026-07-30 | Refactored `StallUpgradeManager.kt` to use a declarative, data-driven architecture. Introduced a `ScalingRule` system and centralized `STAT_RULES` configuration to eliminate redundant strategy objects and switch-based logic. This improves maintainability and reduces allocation overhead during stat calculations. | Resolved |
