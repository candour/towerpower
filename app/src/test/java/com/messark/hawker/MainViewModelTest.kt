...
class MainViewModelTest {
    ...
    fun `availableStalls have correct descriptions`() = runBlocking {
        ...
        assertEquals("Provides $100 every wave", stalls.find { it.name == "ATM" }?.description)
        assertEquals("Boosts adjacent stalls", stalls.find { it.name == "Bak Kut Teh" }?.description)
    }
}
